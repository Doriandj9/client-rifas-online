...
const routes = routesweb.dashboard.children.raffles.children.confirm_payment.children;

const App = () => {
  useSetHeader('Métodos de pago');
  const handler = useMatches();
  const navigate = useNavigate();
  
  useEffect(() => {
    if(handler.length === 3 && handler[2].handle['payment.default']){
        navigate(routes.authorization);
    }
  },[])
  
  if(handler.length === 3 && handler[2].handle['payment.default']){
    return  navigate(routes.authorization);
  }
    return (
        <>
             <div>
